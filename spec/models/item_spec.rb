...
RSpec.describe Item, type: :model do
  ...
  it "発送までの日数の情報がないと登録できない" do
    @item.way_time_id = 0
    @item.valid?
    expect(@item.errors.full_messages).to include("Way time must be other than 0")
  end
  it "価格についての情報がないと登録できない" do
    @item.price = ""
    @item.valid?
    expect(@item.errors.full_messages).to include("Price can't be blank", "Price is not included in the list", "Price 半角数字で入力")
  end
